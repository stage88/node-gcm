        ...
        axios(request_options)
            .then(function(response) {
                ...
                if (status === 200 && data != null) {
                    return callback(null, data, body.registration_ids || [ body.to ]);
                }

                if (data == null) {
                    debug(`Empty response received (${status} ${statusText})`);
                    // Spoof error code 400 to avoid retrying the request
                    return callback({ error: statusText, code: 400 });
                }

                debug(`Invalid request (${status}): ${data}`);
